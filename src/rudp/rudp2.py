...
class Recorder():
    def __init__(self, pkt_period=STAT_PKTS):
        ...
        self.ttlStat = {
            'ttl': [0, [0] * 5]}  # addr  => [sendpkt_num, sendbyte_num, recvpkt_num, recvbyte_num, retransmit_num]
    ...
    def printPeriod(self):
        ...
        for addr, stat in self.timeStat[-1][1].iteritems():
            ...
            print
            '\tSend rate:', rate[0], 'P/s', rate[1], 'B/s'
            print
            '\tRecv rate:', rate[2], 'P/s', rate[3], 'B/s'
            print
            '\tPktl rate:', int((stat[4] / (stat[0] + stat[2])) * 100), '%'
        print
        'All:'
...
class rudpSocket(object):
    ...
    def recvLoop(self):
        while True:
            ...
            # type
            if recvPkt['type'] == DAT:
                self.proDAT(recvPkt, addr)
            elif recvPkt['type'] == ACK:
                self.proACK(recvPkt, addr)
        ...
        # rel
            ...
            try:
                ...
                try:
                    # reset
                    if pktId == 0:
                        self.expId.resetItem(addr, [1])
                    # normal
                    elif pktId == expIdList[-1]:
                        expIdList[-1] += 1
                    # lost packets received
                    else:
                        expIdList.remove(pktId)  # => ValueError
